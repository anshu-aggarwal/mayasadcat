...
Lemmatize = function(word, print =T, debug = F){
  ...
  if(!is.na(word)){
    if(word == ""){
      return("")}}
    lemmax = koRpus::treetag(as.character(word), treetagger="manual", format="obj", debug = debug, TT.tknz=T, lang="en", TT.options=list(path="/Users/mayaaggarwal/Downloads/TreeTagger/bin", preset="en"))
